...
## Toolchain-equivalence comparison

Deferred from Phase 2 (see the
[compatibility ledger](../compat-ledger-rust-1.97.1.md)) and bound here. Before
publication, the frozen public corpus is compiled and verified twice from the
same source: once against an ordinary rustup stable Rust 1.97.1 toolchain and
once against the candidate release bundle, each with isolated target
directories. The comparison requires:

- identical harness inventories, per-harness terminal states, cover results,
  and unreachable-check distributions across the two runs; and
- a byte-identical `Cargo.lock` generated by ordinary stable Cargo 1.97.1 and
  by the bundle's Cargo (the ledger records an explicit negative: no
  `rust-version`, resolver-semantics, or lockfile-format changes in
  1.96–1.97).

Any divergence blocks qualification. The comparison outcome is recorded
alongside the platform gate receipts for the candidate.
